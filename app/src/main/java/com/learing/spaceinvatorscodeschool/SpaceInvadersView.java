...
public class SpaceInvadersView extends SurfaceView implements Runnable {
    ...
    private void update() {
        ...
        if(bullet.getStatus()) {
            for (int i = 0; i < numInvaders; i++) {
                if (invaders[i].getVisibility()) {
                    if (RectF.intersects(bullet.getRectF(), invaders[i].getRect())) {
            ...
            if(invadersBullets[i].getStatus()){
                for(int j = 0; j < numBricks; j++){
                    if(bricks[j].getVisibility()){
                        if(RectF.intersects(invadersBullets[i].getRectF(), bricks[j].getRect())){
        ...
        if(bullet.getStatus()){
            for(int i = 0; i < numBricks; i++){
                if(bricks[i].getVisibility()){
                    if(RectF.intersects(bullet.getRectF(), bricks[i].getRect())){
                        // A collision has occurred
                        bullet.setInactive();
        ...
        for(int i = 0; i < invadersBullets.length; i++){
            if(invadersBullets[i].getStatus()){
                if(RectF.intersects(playerShip.getRect(), invadersBullets[i].getRectF())){
                    invadersBullets[i].setInactive();
                    lives --;
    ...
    private void draw(){
        ...
        if (ourHolder.getSurface().isValid()) {
            ...
            canvas.drawBitmap(playerShip.getBitmap(), playerShip.getX(), screenY - 180, paint);
            // Draw the invaders
            for(int i = 0; i < numInvaders; i++){
                if(invaders[i].getVisibility()) {
                    if(uhOrOh) {
            ...
            canvas.drawText("Score: " + score + "   Lives: " + lives + "   FPS: " + Long.toString(fps), 10, 50, paint);
